...
//  FrancoSphere
//
//  🎯 COMPLETE TYPE DEFINITIONS - ALL 445+ TYPES INCLUDED
...
// MARK: - FrancoSphere Namespace - ALL ACTUAL TYPE DEFINITIONS
public enum FrancoSphere {
    ...
    public struct ContextualTask: Identifiable, Codable {
        ...
        public init(id: String, task: MaintenanceTask, location: NamedCoordinate,
                   weather: WeatherData?, estimatedTravelTime: TimeInterval?, priority: Int) {
            self.id = id
            self.task = task
    ...
    public struct BuildingInsight: Identifiable, Codable {
        ...
        public init(id: String, buildingId: String, title: String, description: String, type: String, priority: String, actionRequired: Bool, createdDate: Date) {
            self.id = id
            self.buildingId = buildingId
            self.title = title
            self.description = description
...
// MARK: - Clean Type Aliases (NO Circular References)
