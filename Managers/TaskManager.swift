...
actor TaskManager {
    ...
    private func createTaskFromCSVData(
        ...
        guard let buildingID = await BuildingRepository.shared.id(forName: buildingName) else {
        ...
        do {
            ...
            if let row = rows.first, let taskId = row["id"] as? Int64 {
                ...
                try await sqliteManager.execute(assignmentQuery, [taskId, workerID])
    ...
    func fetchTasksAsync(forWorker workerId: String, date: Date) async -> [FrancoSphere.MaintenanceTask] {
        ...
        do {
            ...
            GROUP BY t.id
            """
            let rows = try await sqliteManager.query(query, [workerId, "\(dateString)%"])
    ...
    func createTaskAsync(_ task: FrancoSphere.MaintenanceTask) async -> Bool {
        ...
        do {
            ...
            // FIXED: Remove erroneous code and execute the query properly
            let startTimeBinding: SQLite.Binding = startTimeStr != nil ? startTimeStr! as SQLite.Binding : NSNull() as! any Binding as SQLite.Binding
            let endTimeBinding: SQLite.Binding = endTimeStr != nil ? endTimeStr! as SQLite.Binding : NSNull() as! any Binding as SQLite.Binding
            ...
            let parameters: [SQLite.Binding] = [
                ...
                startTimeBinding, endTimeBinding
            ]
    ...
    func createTaskAssignments(taskId: Int64, workerIds: [String]) async throws {
        ...
        do {
            try await sqliteManager.execute("DELETE FROM task_assignments WHERE taskId = \(taskId)")
            for workerId in workerIds {
                try await sqliteManager.execute("INSERT INTO task_assignments (taskId, workerId) VALUES (\(taskId), '\(workerId)')")
            }
    ...
    func taskExistsWithExternalId(_ externalId: String) async -> Bool {
        guard let sqliteManager = sqliteManager else { return false }
        
        do {
            let query = "SELECT COUNT(*) as count FROM tasks WHERE external_id = ?"
            let rows = try await sqliteManager.query(query, [externalId])
            
            if let row = rows.first, let count = row["count"] as? Int64 {
                return count > 0
            }
        } catch {
            print("Error checking for existing external ID: \(error)")
        }
    ...
    func toggleTaskCompletionAsync(taskID: String, completedBy: String = "System") async {
        ...
        do {
            ...
            let statusQuery = "SELECT isCompleted FROM tasks WHERE id = \(taskIdInt)"
            let rows = try await sqliteManager.query(statusQuery, [ ])
            if let row = rows.first {
                let currentStatus = row["isCompleted"] as? Int64 ?? 0
                let newStatus = currentStatus == 0 ? 1 : 0

                // Update task status
                try await sqliteManager.execute("UPDATE tasks SET isCompleted = \(newStatus) WHERE id = \(taskIdInt)")

                // If task is being marked as complete, add to history and handle recurring tasks
                if newStatus == 1 {
                    // Get task details for maintenance history
                    let taskQuery = "SELECT name, buildingId FROM tasks WHERE id = \(taskIdInt)"
                    let taskRows = try await sqliteManager.query(taskQuery, [ ])
                    if let taskRow = taskRows.first,
                       ...
                       let buildingId = taskRow["buildingId"] as? String {
                        ...
                        let workerQuery = "SELECT workerId FROM task_assignments WHERE taskId = \(taskIdInt) LIMIT 1"
                        let workerRows = try await sqliteManager.query(workerQuery, [ ])
                        ...
                        ) VALUES ('\(buildingId)', '\(taskIdInt)', '\(taskName)', 'Task marked as completed',
                                  '\(completionDate)', '\(completedBy)', '\(workerId)')
                        """
                        try await sqliteManager.execute(historyQuery)
                    }
    ...
    private func scheduleNextOccurrenceForTask(taskId: Int64) async {
        ...
        do {
            ...
            FROM tasks
            WHERE id = \(taskId) AND recurrence != 'One Time'
            """
            ...
            if let row = rows.first,
               ...
               let recurrence = FrancoSphere.TaskRecurrence(rawValue: recurrenceStr) {
                ...
                VALUES ('\(name)', '\(description)', '\(buildingId)', '\(nextDateStr)', 0,
                ...
                if let idRow = idRows.first, let newTaskId = idRow["id"] as? Int64 {
                    ...
                    SELECT \(newTaskId), workerId FROM task_assignments
                    WHERE taskId = \(taskId)
                    """
                    try await sqliteManager.execute(assignmentsQuery)
    ...
    func fetchTasksAsync(fromDate: Date, toDate: Date) async -> [FrancoSphere.MaintenanceTask] {
        ...
        do {
            ...
            SELECT t.id, t.name, t.description, t.buildingId, t.scheduledDate, t.isCompleted,
                   t.recurrence, t.urgencyLevel, t.category, t.startTime, t.endTime
            FROM tasks t
            WHERE t.scheduledDate >= '\(fromDateStr)' AND t.scheduledDate <= '\(toDateStr)'
            ORDER BY t.scheduledDate ASC
            """
            let rows = try await sqliteManager.query(query, [ ])
