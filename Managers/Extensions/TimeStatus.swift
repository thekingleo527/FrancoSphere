...
extension TaskManager {
    ...
    func fetchTodaysTasks(forWorker workerId: String) async -> (current: [FrancoSphere.MaintenanceTask], upcoming: [FrancoSphere.MaintenanceTask], completed: [FrancoSphere.MaintenanceTask]) {
        
        let todaysTasks = await fetchTasksAsync(forWorker: workerId, date: Date())
        
        var current: [FrancoSphere.MaintenanceTask] = []
        var upcoming: [FrancoSphere.MaintenanceTask] = []
        var completed: [FrancoSphere.MaintenanceTask] = []
    ...
    func getNextTask(forWorker workerId: String) async -> FrancoSphere.MaintenanceTask? {
...
class TaskStateManager: ObservableObject {
    @Published var currentTasks: [FrancoSphere.MaintenanceTask] = []
    @Published var upcomingTasks: [FrancoSphere.MaintenanceTask] = []
    @Published var completedTasks: [FrancoSphere.MaintenanceTask] = []
    ...
    private func startAutoRefresh() {
        refreshTimer = Timer.scheduledTimer(withTimeInterval: 60, repeats: true) { _ in
            Task { @MainActor [weak self] in
                ...
                var newCurrent: [FrancoSphere.MaintenanceTask] = []
                var newUpcoming: [FrancoSphere.MaintenanceTask] = []
