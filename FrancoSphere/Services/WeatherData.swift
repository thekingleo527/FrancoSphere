...
struct WeatherData: Identifiable {
    ...
    // Generate weather-related tasks based on forecast
    static func generateWeatherRelatedTasks(forBuilding buildingId: String) -> [MaintenanceTask] {
        ...
        if let rainDay = sampleData.first(where: { $0.condition == .rain || $0.condition == .storm }) {
            ...
            weatherTasks.append(MaintenanceTask(
                ...
                dueDate: rainDate,
                startTime: rainDate,
                endTime: calendar.date(byAdding: .hour, value: 2, to: rainDate),
        ...
        if let hotDay = sampleData.first(where: { $0.temperature > 85 }) {
            let taskDate = calendar.date(byAdding: .day, value: -1, to: hotDay.date)!
            ...
            weatherTasks.append(MaintenanceTask(
                ...
                dueDate: taskDate,
                startTime: taskDate,
                endTime: calendar.date(byAdding: .hour, value: 3, to: taskDate),
                category: .maintenance,
                urgency: .high,
                recurrence: .oneTime,
                isComplete: false, assignedWorkers: []
        ...
        if let coldDay = sampleData.first(where: { $0.temperatureLow < 32 }) {
            let taskDate = calendar.date(byAdding: .day, value: -1, to: coldDay.date)!
            ...
            weatherTasks.append(MaintenanceTask(
                ...
                dueDate: taskDate,
                startTime: taskDate,
                endTime: calendar.date(byAdding: .hour, value: 2, to: taskDate),
                category: .maintenance,
                urgency: .high,
                recurrence: .oneTime,
                isComplete: false, assignedWorkers: []
        ...
        if let windyDay = sampleData.first(where: { $0.windSpeed > 20 }) {
            let taskDate = calendar.date(byAdding: .day, value: -1, to: windyDay.date)!
