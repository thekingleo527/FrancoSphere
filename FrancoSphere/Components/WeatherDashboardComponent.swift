...
struct WeatherDashboardComponent: View {
    ...
    // Use WeatherDataAdapter which is already ObservableObject
    @StateObject private var adapter = WeatherDataAdapter.shared
    
    // Remove direct reference to WeatherService since it doesn't conform to ObservableObject
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if isExpanded {
                // Current weather
                if let currentWeather = adapter.currentWeather {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 12) {
                        ForEach(adapter.forecast) { day in
        ...
        .cornerRadius(12)
        .onAppear {
            // Use adapter to fetch weather data
    ...
    // Current weather view component - Updated to use FSWeatherData instead of WeatherData
    private func currentWeatherView(_ weather: FSWeatherData) -> some View {
        HStack(spacing: 15) {
    ...
    // Daily forecast view component - Updated to use FSWeatherData
    private func dailyForecastView(_ weather: FSWeatherData) -> some View {
        VStack(spacing: 6) {
        ...
        .padding(.vertical, 4)
        .padding(.horizontal, 8)
        .background(weather.isHazardous ? Color.red.opacity(0.1) : Color.clear)
        .cornerRadius(8)
    }
    ...
    private func weatherAlertsView() -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // Task rescheduling info - use adapter to get forecast
            if !adapter.forecast.filter({ $0.isHazardous }).isEmpty {
    ...
    private func weatherTasksSheet() -> some View {
        NavigationView {
            VStack {
                ...
                Button(action: {
                    let tasks = adapter.generateWeatherTasks(for: building)
                    TaskManager.shared.createWeatherBasedTasks(for: building.id, tasks: tasks)
