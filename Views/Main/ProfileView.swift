...
struct ProfileView: View {
    ...
    // ✅ Fixed: Computed properties to safely access contextEngine properties
    private var currentWorkerRole: String {
        return contextEngine.currentWorker?.role.capitalized ?? "Worker"
    }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showImagePicker) {
            FrancoSphereImagePicker(isPresented: $showImagePicker, selectedImage: $profileImage)
        }
        ...
        GlassProfileCard(title: "Worker Information") {
            VStack(spacing: 16) {
                ProfileInfoRow(
                    ...
                    label: "Worker ID",
                    value: authManager.workerId
                )
                
                ProfileInfoRow(
                    icon: "building.2.fill",
                    label: "Assigned Buildings",
                    value: "\(contextEngine.getBuildingsCount())"
                )
            ...
            VStack(spacing: 16) {
                HStack(spacing: 16) {
                    ProfileStatCard(  // ✅ Fixed: Renamed to avoid conflict with existing StatCard
                        title: "Tasks Today",
                        value: "\(contextEngine.getTasksCount())",
                        color: .blue
                    )
                    
                    ProfileStatCard(  // ✅ Fixed: Renamed to avoid conflict
                        title: "Completed",
                        value: "\(contextEngine.getCompletedTasksCount())",
                        color: .green
                    )
                }
