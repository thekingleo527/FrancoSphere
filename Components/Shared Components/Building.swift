...
extension FrancoSphere.NamedCoordinate {
    ...
    func distance(from other: FrancoSphere.NamedCoordinate) -> Double {
...
extension Array where Element == FrancoSphere.NamedCoordinate {
    ...
    /// Filter by IDs
    func withIds(_ ids: [String]) -> [FrancoSphere.NamedCoordinate] {
        return self.filter { ids.contains($0.id) }
    }
    
    /// Find by ID
    func building(withId id: String) -> FrancoSphere.NamedCoordinate? {
        return self.first { $0.id == id }
    }
    
    /// Sort by distance from coordinate
    func sortedByDistance(from coordinate: CLLocationCoordinate2D) -> [FrancoSphere.NamedCoordinate] {
